...
void main() async {
  ...
  appState.addListener(() {
    // Xóa dòng debugLogAppState(appState);
  });
  ...
  ErrorWidget.builder = (FlutterErrorDetails details) {
    try {
      ...
      for (final line in stackTrace.split('\n')) {
        if (line.startsWith('packages/chattest/')) {
...
class _MyAppState extends State<MyApp> {
  ...
  void initState() {
    ...
    userStream = chattestFirebaseUserStream()
      ..listen((user) {
        _appStateNotifier.update(user);
        // Xóa dòng debugLogAuthenticatedUser();
      });
    ...
    _router.routerDelegate.addListener(() {
      if (mounted) {
        // Xóa đoạn debugLogGlobalProperty
